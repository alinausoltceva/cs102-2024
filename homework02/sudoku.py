...
def find_possible_values(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.Set[str]:
    ...
    anws = [i for i in values if i in possible_in_row and i in possible_in_col and i in possible_in_block]
...
def check_solution(grid: tp.List[tp.List[str]]) -> bool:
    for i in range(len(grid)):
        for j in range(len(grid[i])):
            ...
            if (
                len(row) == len(sorted(set(row)))
                and len(col) == len(sorted(set(col)))
                and len(block) == len(sorted(set(block)))
            ):
                return True
    else:
        return False
